...
export async function scrapeGoogleMaps(keyword, location, minRating = 0, minReviews = 0, maxResults = 50) {
  ...
  try {
    ...
    console.log('[Scraper] ⚙️  Minimum rating filter:', minRating > 0 ? minRating : 'None');
    console.log('[Scraper] 💬 Minimum reviews filter:', minReviews > 0 ? minReviews : 'None');
    console.log('[Scraper] 📊 Max results:', maxResults);
    ...
    for (const business of businessLinks.slice(0, maxResults)) {
      try {
        ...
        // Wait for detail panel to load - CRITICAL: Wait for the NEW business name to appear
        try {
          // Wait for the h1 element to update with the new business name
          await page.waitForFunction(
            (expectedName) => {
              const nameElement = document.querySelector('h1.DUwDvf, h1.fontHeadlineLarge');
              return nameElement && nameElement.innerText.trim() === expectedName;
            },
            { timeout: 5000 },
            business.name
          );
          // Additional small delay to ensure all content loads
          await page.waitForTimeout(1000);
        } catch (err) {
          // Fallback if name matching fails - just wait
          console.log(`[Scraper]   ⚠️  Name verification failed, using fallback delay`);
        ...
        const reviewsNum = parseInt((lead.reviews || '0').replace(/,/g, '').replace(/\D/g, ''));
        if (minReviews > 0 && (isNaN(reviewsNum) || reviewsNum < minReviews)) {
          console.log(`[Scraper]   ⏭️  Skipped (${reviewsNum} reviews < ${minReviews})`);
          continue;
        }
