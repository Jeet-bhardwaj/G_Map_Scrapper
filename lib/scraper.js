...
export async function scrapeGoogleMaps(keyword, location, minRating = 0, maxResults = 50) {
  ...
  // Add Chrome profile settings for local development
  if (useChromeProfile) {
    try {
      ...
      console.log(`[Scraper] Using Chrome profile: ${profileName}`);
      console.log(`[Scraper] Profile directory: ${userDataDir}`);
      
      launchOptions.userDataDir = userDataDir;
      launchOptions.args.push(`--profile-directory=${profileName}`);
    } catch (error) {
      console.warn(`[Scraper] Could not set up Chrome profile, using guest mode:`, error.message);
    }
  } else {
